...
for epoch in range(EPOCHS):
    ...
    print(f'Epoch = {epoch+1}/{EPOCHS} | Learning Rate = {LR} | time = {time_elapsed // 60:.0f}:{time_elapsed % 60:.0f}')
    ...
    for phase in ['train', 'val']:
        ...
        for data in dataloaders[phase]:
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                loss = LOSS_FNC(m(outputs), labels)
                if phase == 'train':
                    loss.backward()
                    optimizer.step()
...
if isSubmission:
    ...
    submission_path = os.path.join(data_path, f'submission_{datetime.now().strftime("%H%M_%d%m%y")}.csv')
...
files.download(os.path.join(data_path, submission_path))