...
# %% Sort
train_features_decider = []
train_labels_decider = []
...
for feature, label in zip(train_features, train_labels):
    if label in [0, 1]:
        train_features_01.append(feature)
        train_labels_01.append(label)

        train_features_decider.append(feature)
        train_labels_decider.append(0)
    elif label in [2, 3]:
        train_features_23.append(feature)
        train_labels_23.append(label - 2)

        train_features_decider.append(feature)
        train_labels_decider.append(1)

# %% Extreme Gradient Boosting
xgb_classifier_decider = xgb.XGBClassifier(booster='gbtree', max_depth=5, device='gpu')
xgb_classifier_decider.fit(train_features_decider, train_labels_decider)

xgb_classifier_01 = xgb.XGBClassifier(booster='gbtree', max_depth=5, device='gpu')
xgb_classifier_01.fit(train_features_01, train_labels_01)

xgb_classifier_23 = xgb.XGBClassifier(booster='gbtree', max_depth=5, device='gpu')
xgb_classifier_23.fit(train_features_23, train_labels_23)
...
for feature, label in zip(test_features, test_labels):
    if label in [0, 1]:
        test_predictions_01.extend(xgb_classifier_01.predict(np.array([feature])))
        test_labels_01.append(label)
    elif label in [2, 3]:
        test_predictions_23.extend(xgb_classifier_23.predict(np.array([feature])) + 2)
        test_labels_23.append(label)
test_predictions = test_predictions_01 + test_predictions_23
test_labels_new = test_labels_01 + test_labels_23

accuracy, f1, recall, precision, splitted_metrics = get_metrics(test_labels_new, test_predictions)
print(f'--- Ensemble --- \nAccuracy: {accuracy} \nF1: {f1} \nRecall: {recall} \nPrecision: {precision}')
[print(f'{label_metrics}') for label_metrics in splitted_metrics.items()]
...
for feature, label in zip(test_features, test_labels):
    decider_prediction = xgb_classifier_decider.predict(np.array([feature]))
    if decider_prediction in [0]:
        test_predictions_01.extend(xgb_classifier_01.predict(np.array([feature])))
        test_labels_01.append(label)
    elif decider_prediction in [1]:
        test_predictions_23.extend(xgb_classifier_23.predict(np.array([feature])) + 2)
        test_labels_23.append(label)
test_predictions = test_predictions_01 + test_predictions_23
test_labels_new = test_labels_01 + test_labels_23
...
for feature, label in zip(test_features, test_labels):
    prob_01 = xgb_classifier_01.predict_proba(np.array([feature]))[0]
    prob_23 = xgb_classifier_23.predict_proba(np.array([feature]))[0]
    print(prob_01, prob_23)
    
    test_01 = np.abs(prob_01[0] - prob_01[1])
    test_23 = np.abs(prob_23[0] - prob_23[1])
    
    if test_01 > test_23:
        test_predictions_01.extend(xgb_classifier_01.predict(np.array([feature])))
        test_labels_01.append(label)
    elif test_23 > test_01:
        test_predictions_23.extend(xgb_classifier_23.predict(np.array([feature])) + 2)
        test_labels_23.append(label)
test_predictions = test_predictions_01 + test_predictions_23
test_labels_new = test_labels_01 + test_labels_23
...
# %%
# voting_classifier = VotingClassifier(estimators=[], voting='hard', n_jobs=-1)
